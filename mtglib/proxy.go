...
import (
	...
	"github.com/dewadaru/mtg/v2/mtglib/internal/telegram"
...
// Pre-allocate slice capacity for expected connections to reduce reallocations.
// Adjust expectedConnections as appropriate for your workload.
const expectedConnections = 128

// Use sync.Pool for frequently allocated structs (example: Session).
...
type Session struct {
	// Add session-related fields here
}

// getSession and putSession help reuse Session objects.
...
// Example function that handles multiple connections efficiently.
func HandleConnections(listener net.Listener) {
	// Pre-allocate slice with expected capacity.
	connections := make([]net.Conn, 0, expectedConnections)
	...
	for {
		...
		// Use pooled Session for each connection.
		session := getSession()
		go func(c net.Conn, s *Session) {
			defer c.Close()
			defer putSession(s)
			// ... handle session ...
		}(conn, session)
